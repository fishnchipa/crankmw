...
export default function Navbar({ menu, setMenuAction }: NavbarProps) {
  ...
  // TODO: Optimize and fix sliding bar
  return (
    <nav
      style={{
    ...
    >
      {!isBurger ? (
        <div className="w-full h-full flex items-center justify-between px-5 relative md:justify-center">
          {!isWindow && (isScrolled || active) && (
            <motion.div className="absolute left-5" {...iconAnimate}>
          ...
          <ul className="h-full flex flex-row items-center relative font-noto-sans font-normal text-white">
            ...
            <Selector left={selector.left} width={selector.width} />
          </ul>
          {(isWindow || isScrolled || active) && (
            <motion.div className="absolute right-5" {...iconAnimate}>
              <Anchor href="/cart">
                <ShoppingCart />
              </Anchor>
            </motion.div>
        ...
        <div className="w-full h-full flex items-center justify-between px-5">
          <MenuButton isOpen={menu} onClick={toggleHamburger} />
          <div className="w-[110px] h-[45px] relative">
            <Image src="/logo.webp" alt="logo" className="object-cover" fill />
          </div>
        </div>
      )}
    </nav>
